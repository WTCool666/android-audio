...
public class BasePermissionActivity extends AppCompatActivity {
    ...
    public void requestPermission(final Activity activity, final int requestCode, PermissionsGrant permissionGrant) {
        if (activity == null) {
            Log.v(TAG,"requestPermission acitvity is null!!!");
            return;
        }

        Log.v(TAG,"requestPermission requestCode:" + requestCode);
        if (requestCode < 0 || requestCode >= requestPermissions.length) {
            Log.v(TAG,"requestPermission illegal requestCode:" + requestCode);
            return;
        }
...
//        }
        ...
        try {
            ...
            Toast.makeText(activity, "please open this permission:"+requestPermission, Toast.LENGTH_SHORT)
                    .show();
            Log.v(TAG,"requestPermission RuntimeException:" + e.getMessage());
            return;
        }

        if (checkSelfPermission != PackageManager.PERMISSION_GRANTED) {
            Log.v(TAG, "requestPermission startRequestPermission");
            if (ActivityCompat.shouldShowRequestPermissionRationale(this,requestPermission)) {
                Log.v(TAG, "requestPermission shouldShowRequestPermissionRationale");
                shouldShowRationale(activity, requestCode, requestPermission);
            } else {
                Log.v(TAG, "requestPermission Permission granted is prohibited");
                ActivityCompat.requestPermissions(this,new String[]{requestPermission}, requestCode);
            }

        } else {
            Log.v( TAG, "requestPermission Permission has been granted");
    ...
    public void requestMultiPermissions(final Activity activity, PermissionsGrant grant) {
        ...
        Log.v(TAG,  "requestMultiPermissions permissionsList:" + permissionsList.size() + ",shouldRationalePermissionsList:" + shouldRationalePermissionsList.size());
    ...
    public void requestPermissionsResult(final Activity activity, final int requestCode, @NonNull String[] permissions,
        ...
        if (activity == null) {
            Log.v(TAG,"requestPermissionsResult acitvity is null!!!");
            return;
        }
        Log.v(TAG, "requestPermissionsResult requestCode:" + requestCode);
        ...
        if (requestCode < 0 || requestCode >= requestPermissions.length) {
            Log.v( TAG, "requestPermissionsResult illegal requestCode:" + requestCode);
            return;
        }

        Log.v(TAG, "requestPermissionsResult requestCode:" + requestCode + ",permissions:" + permissions[0]
    ...
    private void openSettingActivity(final Activity activity, String message) {
        showMessageOKCancel(activity, message, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                Log.v(TAG, "getPackageName(): " + activity.getPackageName());
        ...
        for (int i = 0; i < requestPermissions.length; i++) {
            ...
            try {
                ...
                Toast.makeText(activity, "please open those permission", Toast.LENGTH_SHORT)
                        .show();
                Log.v(  TAG,"getNoGrantedPermission RuntimeException:" + e.getMessage());
                return null;
            }

            if (checkSelfPermission != PackageManager.PERMISSION_GRANTED) {
                Log.v(TAG, "getNoGrantedPermission != PackageManager.PERMISSION_GRANTED:" + requestPermission);
                if (ActivityCompat.shouldShowRequestPermissionRationale(this,requestPermission)) {
                    Log.v(TAG, "getNoGrantedPermission shouldShowRequestPermissionRationale");
                    ...
                    Log.v( TAG,"getNoGrantedPermission Permission granted is prohibited");
                }
            }
        }
        return permissions;
