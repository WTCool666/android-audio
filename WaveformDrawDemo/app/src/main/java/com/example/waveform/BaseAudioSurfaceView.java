...
//	private final int channelCount=1;
	//每个屏幕绘制多少个采样点
	private float drawSamplerCountPreScreen = (float)(samplerRate/INDEX_TIMES*SECOND_PRESCREEN);
	//每隔多远描绘一次，默认一个屏幕只描绘10s*16000/100次
...
//        LogUtils.v("1 divider="+divider);
		drawSamplerCountPreScreen = (float) (samplerRate/INDEX_TIMES*SECOND_PRESCREEN);
...
//	private float
	private void setAudioParam(int samplerRate,int bitWidth,boolean  rightChannel,int fileLength){
		this.rightChannel=rightChannel;
		this.samplerRate=samplerRate;
		this.bitWidth=bitWidth;
//		LogUtils.v("width="+width+" getWidth()="+getWidth()+" height="+height+" getHeight()="+getHeight());
		...
		MAX_HEIGHT=getHeight()-MARKER_LINE;
//        LogUtils.v("1 divider="+divider);
		float tempCount=fileLength/(bitWidth/8)/INDEX_TIMES;
//					tempCount=(float) (tempIntBuf.length/INDEX_TIMES);
		LogUtils.v("tempCount="+tempCount+" drawSamplerCountPreScreen="+drawSamplerCountPreScreen);
		if (tempCount>drawSamplerCountPreScreen){
			drawSamplerCountPreScreen=tempCount;
		}
		divider = (float) (MAX_WIDHT/drawSamplerCountPreScreen);
//		LogUtils.v("divider="+divider);
		yAxisTimes = getYAxisTimesByBitWidth(bitWidth);
	...
	private Thread caculateSamplerThread=null;
	private void addAudioDatas(byte[] tempDatas,int fileLength, int samplerRate,int bitWidth, boolean rightChannel){
		setAudioParam(samplerRate,bitWidth ,rightChannel,fileLength );
		setWaveformPaintColor(R.color.waveformBorderLine);
		if (caculateSamplerThread==null){
			AudioDataDrawRunnalbe mAudioDataDrawRunnalbe=new AudioDataDrawRunnalbe(tempDatas, fileLength,bitWidth );
			caculateSamplerThread=new Thread(mAudioDataDrawRunnalbe);
			caculateSamplerThread.setName("caculateSamplerThread");
			caculateSamplerThread.start();
		}
	}
...
//		getTargetAudioBuf(AudioUtils.byteArray2SamplerArray(data,size ,bitWidth ));
	...
	private float getYAxisTimesByBitWidth(int bitWidth){
		...
		switch (bitWidth){
			case BIT_8_WIDTH:
				tempYAxisTimes=(float)(Byte.MAX_VALUE/MAX_HEIGHT);
				break;
			case BIT_16_WIDTH:
				tempYAxisTimes=(float)(Short.MAX_VALUE/ MAX_HEIGHT);
				break;
			case BIT_24_WIDTH:
				tempYAxisTimes=(float)(8388607/MAX_HEIGHT);
				break;
			case BIT_32_WIDTH:
				tempYAxisTimes=(float)(Integer.MAX_VALUE/MAX_HEIGHT);
				break;
		}
...
//			LogUtils.v("tempData="+tempData+" i="+i);
	...
	private int[] byteArray2SamplerArrays(byte[] data,int size){
		int byteCountPreSampler = (bitWidth /8);
		if (data==null || size==0){
			return null;
		}
		...
		int tempData =0;
		int[] tempSamplerData=new int[(int) drawSamplerCountPreScreen];

		LogUtils.v("samplerCount="+samplerCount+" byteCountPreSampler="+byteCountPreSampler);
		int j=0;
		int z=0;
		for(int i=0;i<samplerCount;i+=INDEX_TIMES){
			tempData=0;
			for(int k=0;k<byteCountPreSampler;k++){
				int tempBuf =0;
				if ((j+k)<data.length){
...
//			LogUtils.v("tempData="+tempData+" i="+i);
			tempSamplerData[z]=tempData;
			z++;
			if (z>=drawSamplerCountPreScreen){
				break;
			}
//			LogUtils.v("i="+i+" tempSamplerData.len="+tempSamplerData.length);
			j+=(byteCountPreSampler*INDEX_TIMES);
		}


		/*for(int i=0;i<samplerCount;i++){
			for(int j=0;j< (data.length%2==0?data.length:data.length+1);j+=byteCountPreSampler){
				for(int k=0;k<byteCountPreSampler;k++){
					tempData = ( data[j+k] << (k*8) );
					tempData |=tempData;
				}
				LogUtils.v("i="+i);
				tempSamplerData[i]=tempData;
...
//		LogUtils.v("getTargetAudioBuf");
		synchronized (inBuf) {
			for (int i = 0; i < tempBuf.length; i += INDEX_TIMES) {
				if (isRunning){
					if (rightChannel){
						if( (i+1) <tempBuf.length ){
							inBuf.add(tempBuf[i+1]);
...
//		LogUtils.v("========exit draw=============");
	...
	public void stopDrawsThread(){
		if (drawsThread!=null || isRunning==true || caculateSamplerThread!=null){
			isRunning=false;
			if (!inBuf.isEmpty()){
				inBuf.clear();
			}
			drawsThread=null;
			caculateSamplerThread=null;
...
//		}
	...
	private class AudioDataDrawRunnalbe implements  Runnable{
		private byte[]tempDatas;
		private int fileLength;
		private int bitWidth;
		public AudioDataDrawRunnalbe(byte[]tempDatas,int fileLength ,int bitWidth){
		...
		@Override
		public void run() {
			int[] tempInBuf = byteArray2SamplerArrays(tempDatas,fileLength);
//			int[] tempInBuf = AudioUtils.byteArray2SamplerArray(tempDatas,fileLength ,bitWidth );
			tempDatas=null;
			getTargetAudioBufs(tempInBuf);
		}
	}
